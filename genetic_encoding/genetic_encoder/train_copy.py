...
from torch.utils.data import DataLoader, TensorDataset, DistributedSampler, IterableDataset
...
def main(rank, world_size):
    ...
    embed_dim     = 512
    mlm_epochs    = 1       # Change back to 10
    clf_epochs    = 100
    batch_size    = 10      # Change back to 64
    num_workers   = 32
    max_len       = 2048
    ...
    # — DataLoaders —
    train_loader = get_dataloader(base_ds, batch_size=batch_size, num_workers=num_workers, rank=rank, world_size=world_size)
    val_loader = get_dataloader(val_ds, batch_size=batch_size, num_workers=num_workers, rank=rank, world_size=world_size)
    test_loader = get_dataloader(test_ds, batch_size=batch_size, num_workers=num_workers, rank=rank, world_size=world_size)
    ...
    for epoch in mlm_pbar:
        ...
        train_emb, train_lab = cache_embeddings(train_loader); cache_pbar.update(1); torch.cuda.empty_cache()
        val_emb, val_lab = cache_embeddings(val_loader); cache_pbar.update(1); torch.cuda.empty_cache()
        test_emb, test_lab = cache_embeddings(test_loader); cache_pbar.update(1); torch.cuda.empty_cache()
        cache_pbar.close()
        torch.cuda.empty_cache()
