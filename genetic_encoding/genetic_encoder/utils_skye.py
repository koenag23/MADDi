...
class VCFDataset(IterableDataset):
    ...
    def _build_sequences(self):
        ...
        for subj in self.subjects:
            tokens = []
            cells = self.df[subj].fillna('None')
            if self.total_snps is not None:
                cells = cells.iloc[:self.total_snps]
            ...
            for idx, cell in cells.items():
                ...
                snp_id= row.get('ID', None)
                ref   = row.get('REF', None)
                alt   = row.get('ALT', None)
                qual  = row.get('QUAL', None)
    ...
    def __iter__(self):
        ...
        for ptid in self.subjects:
            ids = [self.token_to_id.get(tok, pad_id)
                   for tok in self.subject_sequences[ptid]]
