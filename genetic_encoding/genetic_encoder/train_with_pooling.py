...
from MADDi.genetic_encoding.genetic_encoder.utils_old import TokenizedVCFDataset, mask_inputs
import random
import numpy as np
...
def extract_embeddings(data_loader, model, token_to_id, id_to_token, field_mask_probs):
    ...
    for batch_tokens, _ in data_loader:
        with torch.no_grad():
            ...
            # Mean pooling last 4 layers' CLS tokens
            last_4_layers = outputs.hidden_states[-4:]  # List of tensors
            cls_tokens = [layer[:, 0, :] for layer in last_4_layers]  # Extract CLS from each
            pooled_cls = torch.mean(torch.stack(cls_tokens), dim=0)   # Mean over layers

        embeddings.append(pooled_cls.cpu().numpy())
